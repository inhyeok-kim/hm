...
INSERT INTO item_category (create_at, name, parent_id, family_id, is_root)
VALUES (now(),'Food', 0, 1, 1)
    ,(now(),'냉장',1,1,0)
    ,(now(),'Consumables',0,1,1)
    ,(now(),'티슈',3,1,0)
    ,(now(),'Living',0,1,1)
    ,(now(),'가구',5,1,0)
;

INSERT INTO item (create_at, name,family_id, type,cnt, class_type,category_id)
VALUES (now(), '휴지', 1, '1', 5,'2',4)
    ,(now(), '물티슈', 1, '1', 3,'2',4)
    ,(now(), '손톱깍이', 1, '2', 3,'1',5)
    ,(now(), '일룸침대', 1, '2', 1,'3',6)
    ,(now(), 'LG TV', 1, '2', 1,'3',6)
